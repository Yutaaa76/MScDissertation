...
from nets import HeteroGraphSAGE
...
from dgl.dataloading import neighbor_sampler
...
def train_graph_model(task: 'tweet',
                      size: 'small',
                      num_epochs: int = 300,
    ...
    # Otherwise, build the graph and save it
        ...
        # Build the graph
        graph = get_dgl_data.get_data(size=size)

        # Save the graph
        save_dgl_graph(graph, graph_path)

    # Store labels and masks
    train_mask = graph.nodes['tweet'].data['train_mask'].bool()
    val_mask = graph.nodes['tweet'].data['val_mask'].bool()
    test_mask = graph.nodes['tweet'].data['test_mask'].bool()
    ...
    # Initialise model
    model = HeteroGraphSAGE.HeteroGraphSAGE(input_dropout=0.2,
                                            dropout=0.2,
                                            hidden_dim=1024,
                                            feat_dict=feat_dict,
                                            task=task)
    ...
    if random_split:
        ...
        torch_gen = torch.Generator().manual_seed(990706)
    ...
    # Set up the sampler
    sampler = neighbor_sampler.MultiLayerNeighborSampler([100, 100], replace=False)

    # Set up the dataloaders
    train_dataloader = DataLoader(graph=graph,
                                  indices=train_nids,
                                  graph_sampler=sampler,
                                  batch_size=64,
                                  shuffle=True,
                                  drop_last=False)
    val_dataloader = DataLoader(graph=graph,
                                indices=val_nids,
                                graph_sampler=sampler,
                                batch_size=1000000,
                                shuffle=False,
                                drop_last=False)
    test_dataloader = DataLoader(graph=graph,
                                 indices=test_nids,
                                 graph_sampler=sampler,
                                 batch_size=1000000,
                                 shuffle=False,
    ...
    for epoch in epoch_pbar:
        ...
        stats = [
            ...
            ('train_misinfo_f1', train_misinformation_f1),
            ('train_factual_f1', train_factual_f1),
            ('val_loss', val_loss),
            ('val_misinfo_f1', val_misinformation_f1),
            ('val_factual_f1', val_factual_f1),
            ('test_loss', test_loss),
            ('val_misinfo_f1', test_misinformation_f1),
        ...
        desc = (f'Training - '
                ...
                f'test_factual_f1 {test_factual_f1:.3f} - '
                f'test_misinfo_f1 {test_misinformation_f1:.3f}'
                )
...
train_graph_model(task='tweet', size='medium')
