...
def get_data(size):
    ...
    # print(tweet_claim_article_df.columns)
    tweet_claim_article_df = tweet_claim_article_df[['tweet_id', 'text_emb_x', 'title_emb', 'content_emb', 'claim_emb', 'lang_emb_x',
                                                     'label', 'train_mask', 'val_mask', 'test_mask']]
    tweet_claim_article_df.rename(columns={'text_emb_x': 'text_emb',
                                           'lang_emb_x': 'lang_emb'}, inplace=True)

    # tweet_claim_image_article_df = tweet_claim_article_df.merge(tweet_image_df, how='inner', on='tweet_id')
    # print(tweet_claim_image_article_df.columns)
    tweet_claim_article_df = tweet_claim_article_df[[
        'text_emb', 'label', 'claim_emb', 'content_emb', 'title_emb', 'lang_emb',
        'train_mask', 'val_mask', 'test_mask'
    ]]
    # tweet_claim_article_df.rename(columns={'text_emb_x': 'text_emb'}, inplace=True)
    # print(tweet_claim_image_article_df.columns)

    train_set = tweet_claim_article_df.query('train_mask == True')
    ...
    # return tweet_claim_article_df.query('train_mask == True'), tweet_claim_article_df.query('val_mask == True'), tweet_claim_article_df.query('test_mask == True')
    return train_set, tweet_claim_article_df.query('val_mask == True'), tweet_claim_article_df.query('test_mask == True')
